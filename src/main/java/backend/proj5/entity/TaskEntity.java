...
@NamedQuery(name="Task.totalTasksDoneByEachDay", query="SELECT a.conclusionDate, (SELECT COUNT(b) FROM TaskEntity b WHERE b.stateId = 300 AND b.conclusionDate <= a.conclusionDate) FROM TaskEntity a WHERE a.stateId = 300 AND a.erased = false GROUP BY a.conclusionDate ORDER BY a.conclusionDate ASC")
